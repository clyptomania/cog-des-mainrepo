...
public class PluxDeviceManager {
    ...
    public long[][] GetPackageOfData(bool rebootMemory) {
    ...
    // rebootMemory -> When true the stored data inside BufferedSamples object is re-initialized.
    public long[] GetPackageOfData(int channelNbr, List<int> activeChannelsMask, bool rebootMemory) {
        // Lock is an essential step to ensure that variables shared by the same thread will not be accessed at the same time.
        BufferAcqSamples bufferedSamples = LazyObject.Value;
        lock (bufferedSamples) {
            if (!bufferedSamples.getUncaughtExceptionState()) {
                ...
                long[][] fullData = bufferedSamples.getPackages(rebootMemory);

                // Selection of data inside the desired channel.
                if (fullData != null && fullData[0] != null) {
                    long[] dataInChannel = new long[fullData.Length];
    ...
    bool CallbackHandler(int nSeq, int[] data, int dataLength) {
        ...
        lock (bufferedSamples) {
            ...
            bufferedSamples.addSamples(nSeq, data, DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond);
        }
        return true;
    }
    ...
    public class BufferAcqSamples {
        ...
        public BufferAcqSamples() {
            packagesOfData = new long[maxNbrSamples][]; // Stores 10 seconds of data in data acquisitions of 1000 Hz sampling rate.
        }

        // An important method that ensures the reinitialisation of the class variables.
        public void reinitialise() {
            comCounter = 0;
            packagesOfData = new long[maxNbrSamples][];
        ...
        public void addSamples(int nSeq, int[] newPackage, long unityTS = 0) {
            ...
            // Debug.Log("Got data package with TS " + unityTS);
            ...
            packagesOfData[comCounter] = new long[newPackage.Length + 1];

            packagesOfData[comCounter][0] = unityTS;

            for (int i = 1; i <= newPackage.Length; i++) {
                packagesOfData[comCounter][i] = (long)newPackage[i - 1];
            }
            // packagesOfData[comCounter][newPackage.Length] = unityTS;

            // packagesOfData[comCounter] = newPackage.Select(item => (long)item).ToArray();
        ...
        // rebootMemory -> When true the stored data inside BufferedSamples object is re-initialized.
        public long[][] getPackages(bool rebootMemory) {
            ...
            // Send the filled section of the array.
                ...
                long[][] tempArray = new long[comCounter][];
